...
Status PunchHoleGCJob::HolePunchSingleBlobFile(
  ...
  int fd = open(BlobFileName(db_options_.dirname, file->file_number()).c_str(),
                O_WRONLY);
  if (fd < 0) {
    return Status::IOError("Failed to open blob file for hole punch",
                           strerror(errno));
  }
  // Ensure the fd is closed on every return path below.
  struct FdCloser {
    int fd;
    ~FdCloser() {
      if (fd >= 0) {
  ...
  for (; iter->Valid(); iter->Next()) {
    ...
    // blob_index.blob_handle.size aligned to block size.
    if (fallocate(fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE,
                  blob_index.blob_handle.offset, aligned_data_size) != 0) {
      return Status::IOError("Hole punch failed", strerror(errno));
    }
#else
  ...
  struct stat st;
  if (fstat(fd, &st) == 0) {
    disk_usage_map_[file->file_number()] =
        static_cast<int64_t>(st.st_blocks) * 512;
  }
